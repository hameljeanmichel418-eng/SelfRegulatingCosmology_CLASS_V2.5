...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /** 8.a.3) Self-Regulating Cosmology energy transfer term */
  /* Read */
  class_call(parser_read_string(pfc,"use_src",&string1,&flag1,errmsg),
             errmsg,
             errmsg);
  if (flag1 == _TRUE_){
    if (string_begins_with(string1,'y') || string_begins_with(string1,'Y')){
      pba->use_src = _TRUE_;
      class_read_double("src_gamma0",pba->src_gamma0);
      class_read_double("src_a_peak",pba->src_a_peak);
      class_read_double("src_width",pba->src_width);
    }
    else {
      pba->use_src = _FALSE_;
    }
...
int input_default_params(struct background *pba,
  ...
  /** 9.a.2.2) 'EDE' case */
  pba->Omega_EDE = 0.;
  /** 9.a.3) Self-Regulating Cosmology */
  pba->use_src = _FALSE_;
  pba->src_gamma0 = 0.;
  pba->src_a_peak = 0.8;
  pba->src_width = 0.1;
