...
/**
 * Self-Regulating Cosmology energy transfer term Q(a).
 *
 * Q(a) = src_gamma0 * exp(-0.5 * (ln(a/a_peak)/src_width)^2)
 *
 * It represents a late-time, smooth, positive energy transfer from PBHs
 * to the dark-energy fluid, behaving as a Gaussian in ln(a) centered at
 * a ≈ 0.80 (z ≈ 0.25). It enters only in the dark-energy continuity
 * equation when use_src == TRUE and has_fld == TRUE.
 *
 * @param a   Input: current value of scale factor (in fact, with our conventions, of (a/a_0))
 * @param pba Input: pointer to background structure
 * @return Q(a): energy transfer term
 */

double background_src_Q_of_a(double a, struct background * pba, double rho_fld) {

  double x = log(a / pba->src_a_peak);
  double Gauss = pba->src_gamma0
                 * exp(-0.5 * (x / pba->src_width) * (x / pba->src_width));

  // Q(a) has same units as rho_fld
  return Gauss * rho_fld;
...
int background_derivs(
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    /** - Add Self-Regulating Cosmology energy transfer term Q(a) if enabled */
    if ((pba->use_src == _TRUE_) && (pba->has_fld == _TRUE_)) {
      dy[pba->index_bi_rho_fld] += background_src_Q_of_a(a, pba, pvecback[pba->index_bg_rho_fld]);
    }
  }
